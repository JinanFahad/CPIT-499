...
import { FileText, Sparkles, CheckCircle, MapPin, Loader2 } from "lucide-react";
...
const inputClass =
  "w-full bg-gray-50 dark:bg-[#062620] border-gray-300 dark:border-white/20 text-[#08312d] dark:text-white placeholder:text-gray-400 dark:placeholder:text-white/40 rounded-lg px-4 py-3 text-base font-medium font-[Changa] focus:ring-2 focus:ring-[#C6A75E] focus:border-[#C6A75E]";
const labelClass =
  "block text-[#08312d] dark:text-white font-bold text-base mb-2 font-[Changa]";
const selectClass =
  "w-full bg-gray-50 dark:bg-[#062620] border border-gray-300 dark:border-white/20 text-[#08312d] dark:text-white rounded-lg px-4 py-3 text-base font-medium font-[Changa] focus:ring-2 focus:ring-[#C6A75E] focus:border-[#C6A75E] focus:outline-none appearance-none cursor-pointer";
const sectionTitle =
  "text-[#08312d] dark:text-white font-bold text-lg mb-5 pb-2 border-b border-gray-200 dark:border-[#C6A75E]/30 font-[Changa]";
...
export default function FeasibilityStudyPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.lat || !formData.lng) {
      setError(
        isAr
          ? "يجب تحديد موقع المشروع على الخريطة"
  ...
  return (
    <>
      ...
      >
        ...
        <div className="max-w-3xl mx-auto space-y-6">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-gray-500 dark:text-gray-600 text-sm font-[Changa] mt-2">
                  {isAr
                    ? "أدخل بيانات مشروعك لإنشاء دراسة جدوى شاملة"
                    : "Enter your project data to generate a full study"}
                </p>
              </div>
            </div>
          </motion.div>
          ...
          >
              ...
              <div className="grid grid-cols-1 gap-5 mb-5">
                <div>
                  ...
                  <Input
                    ...
                    placeholder={
                      isAr
                        ? "مثال: مطعم برجر الذهبي"
                        : "e.g. Golden Burger Restaurant"
                    }
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                  ...
                  <div className="relative">
                    ...
                    >
                      <option value="">
                        {isAr ? "اختر نوع المشروع" : "Select business type"}
                      </option>
                      {businessTypes.map((b) => (
            ...
            <div>
              <h2 className={sectionTitle}>
                {isAr ? "٢. تفاصيل المشروع" : "2. Project Details"}
              </h2>
              <div className="grid grid-cols-1 gap-5">
                <div>
                  ...
                  <textarea
                    ...
                    placeholder={
                      isAr
                        ? "اكتبي وصفاً مختصراً عن مشروعك، فكرته، ومميزاته"
                        : "Briefly describe your project, its concept, and what makes it unique"
                    }
                    className={inputClass}
                  />
                </div>
                <div>
                  ...
                  <Input
                    ...
                    placeholder={
                      isAr
                        ? "مثال: الشباب والعائلات في المنطقة"
                        : "e.g. youth and families in the area"
                    }
                    className={inputClass}
                  />
                </div>
                <div>
                  <label className={labelClass}>
                    {isAr
                      ? "المنتجات الرئيسية (اختياري)"
                      : "Main Products (Optional)"}
                  </label>
                  <textarea
                    name="mainProducts"
            ...
            <div>
              <h2 className={sectionTitle}>
                {isAr ? "٣. التفاصيل الاستثمارية" : "3. Investment Details"}
              </h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                  <label className={labelClass}>
                    {isAr ? "رأس المال الأولي (ر.س)" : "Initial Capital (SAR)"}{" "}
                    <span className="text-red-500">*</span>
                  </label>
                  <Input
                    type="number"
                    ...
                    min={5000}
                    placeholder={isAr ? "مثال: 100000" : "e.g. 100000"}
                    className={inputClass}
                    required
                  />
                </div>
                <div>
                  <label className={labelClass}>
                    {isAr ? "الإيجار الشهري (ر.س)" : "Monthly Rent (SAR)"}{" "}
                    <span className="text-red-500">*</span>
                  </label>
                  <Input
                    type="number"
                    ...
                    onKeyDown={preventNegativeKeys}
                    min={1000}
                    placeholder={isAr ? "مثال: 8000" : "e.g. 8000"}
                    className={inputClass}
                    required
                  />
                </div>
            ...
            <div>
              <h2 className={sectionTitle}>
                {isAr ? "٤. التشغيل" : "4. Operations"}
              </h2>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                <div>
                  <label className={labelClass}>
                    {isAr ? "عدد الموظفين" : "No. of Employees"}{" "}
                    <span className="text-red-500">*</span>
                  </label>
                  <Input
                    type="number"
                ...
                <div>
                  <label className={labelClass}>
                    {isAr
                      ? "متوسط سعر المنتج (ر.س)"
                      : "Avg Product Price (SAR)"}{" "}
                    <span className="text-red-500">*</span>
                  </label>
                  <Input
                    type="number"
                    name="avgProductPrice"
                    value={formData.avgProductPrice}
                    onChange={handleChange}
                    onWheel={preventWheelChange}
                    onKeyDown={preventNegativeKeys}
                    min={5}
                ...
                <div>
                  <label className={labelClass}>
                    {isAr ? "عملاء متوقعون يومياً" : "Expected Customers/Day"}{" "}
                    <span className="text-red-500">*</span>
                  </label>
                  <Input
                    type="number"
                    name="expectedCustomersPerDay"
                    value={formData.expectedCustomersPerDay}
                    onChange={handleChange}
                    onWheel={preventWheelChange}
                    onKeyDown={preventNegativeKeys}
                    min={1}
            ...
            <div className="flex flex-col sm:flex-row gap-3 pt-6 border-t-2 border-gray-200 dark:border-[#C6A75E]/20">
              <Button
                type="submit"
                disabled={loading}
                className="flex-1 h-14 bg-[#C6A75E] hover:bg-[#a88f4e] text-white font-bold text-base rounded-lg transition-all font-[Changa]"
              >
      ...
      <LoadingModal
        open={loading}
        title={isAr ? "جاري التوليد" : "Generating"}
        description={
          isAr
            ? "جاري إنشاء دراسة الجدوى الخاصة بمشروعك"
