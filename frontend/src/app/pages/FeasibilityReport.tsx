...
export default function FeasibilityReport() {
    ...
    const load = async () => {
      try {
        ...
        const repRes = await fetch(
          `${BACKEND_URL}/api/reports/${proj.report_id}`,
        );
  ...
  // Map AI classifications (Arabic + English) to banner colors.
  // Supports legacy reports stored with older labels.
  const cls = dec.classification;
  const clsLower = cls.toLowerCase();
  const clsDisplay = tr(cls, language);
  const decisionTheme =
    cls.includes("نجاح مرتفع") ||
    cls.includes("مناسب") ||
    clsLower.includes("suitable") ||
    clsLower.includes("high success")
      ...
      : cls.includes("نجاح محتمل") || clsLower.includes("probable success")
        ...
        : cls.includes("متوسط") ||
            clsLower.includes("moderate") ||
            cls.includes("بشروط") ||
            clsLower.includes("viable with conditions")
  ...
  const costData = [
    {
      name: isAr ? "مرافق" : "Utilities",
      value: fs.utilities_cost,
      color: "#3b82f6",
    },
    {
      name: isAr ? "تشغيل" : "Overhead",
      value: fs.overhead_cost,
      color: "#8b5cf6",
    },
    {
      name: isAr ? "تسويق" : "Marketing",
      value: fs.marketing_cost,
      color: "#C6A75E",
    },
  ];
  ...
  const financialBars = [
    {
      name: isAr ? "إيراد" : "Revenue",
      value: fs.monthly_revenue,
      color: "#22c55e",
    },
    {
      name: isAr ? "مصاريف" : "Expenses",
      value: fs.monthly_expenses,
      color: "#ef4444",
    },
    {
      name: isAr ? "ربح" : "Profit",
      value: fs.monthly_net_profit,
      color: "#08312D",
    },
  ];
  ...
  const tabs: Array<{ key: TabKey; label: string; icon: any }> = [
    {
      key: "financial",
      label: isAr ? "التحليل المالي" : "Financial",
      icon: DollarSign,
    },
        ...
        <div className="report-paper max-w-6xl mx-auto bg-[#F5F7F9] rounded-3xl p-6 lg:p-8 border border-gray-200 dark:border-[#C6A75E]/30 shadow-xl space-y-6 relative z-10">
          ...
          >
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
              <div className="flex-1">
                ...
                <h1 className="text-3xl font-bold text-white mb-2 font-[Changa]">
                  {trEmbeddedCities(report.title, language)}
                </h1>
                ...
                >
                  <div className="flex items-start justify-between flex-wrap gap-4">
                    <div className="flex-1 min-w-[260px]">
                      ...
                      <div
                        className="text-2xl font-black mb-2 font-[Changa]"
                        style={{ color: theme.text }}
                      >
                        {sp.outcome_emoji} {tr(sp.outcome, language)}
                      </div>
                      <p className="text-sm text-gray-700 font-[Changa] leading-relaxed">
                        {tr(sp.message, language)}
                      </p>
                    </div>
                    <div className="flex flex-col items-center gap-2 min-w-[140px]">
                      <div
                        className="text-5xl font-black font-[Changa]"
                        style={{ color: theme.text }}
                      >
                        {sp.score}
                      ...
                      <div className="w-full h-3 bg-gray-200 rounded-full overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${sp.score_percent}%`,
                            background: theme.bar,
                          }}
                        />
                      </div>
                      <p className="text-xs text-gray-500 font-[Changa]">
                        {isAr
                          ? "درجة النجاح المرجّحة"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-2">
                      {sp.factors.map((f) => {
                        ...
                        return (
                          ...
                          >
                            <div className="text-xs text-gray-600 font-[Changa] mb-1 line-clamp-2">
                              {tr(f.name, language)}
                            </div>
                            <div className="flex items-baseline justify-between mb-1">
                              ...
                              <span className="text-xs text-gray-500 font-[Changa]">
                                {tr(f.rating, language)}
                              </span>
                            </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <h3 className="text-red-900 font-bold text-lg mb-2 font-[Changa]">
                    {isAr
                      ? "المشروع غير مجدي بهذي الأرقام"
                      : "Project Not Viable With These Numbers"}
                  </h3>
                  ...
                  <div className="bg-white/70 rounded-lg p-3 border border-red-200">
                    <p className="text-red-900 text-xs font-bold mb-2 font-[Changa]">
                      {isAr
                        ? "💡 جربي تعديل المدخلات:"
                        : "💡 Try adjusting these inputs:"}
                    </p>
                    <ul className="text-red-800 text-xs space-y-1 font-[Changa]">
                      <li>
                        {isAr
                          ? "• قللي عدد الموظفين (الرواتب أكبر بند مصروف)"
                          : "• Reduce employee count (salaries are the biggest expense)"}
                      </li>
                      <li>
                        {isAr
                          ? "• زيدي عدد العملاء المتوقعين يومياً"
                          : "• Increase expected daily customers"}
                      </li>
                      <li>
                        {isAr
                          ? "• ارفعي متوسط سعر المنتج لو واقعي"
                          : "• Increase average product price if realistic"}
                      </li>
                      <li>
                        {isAr
                          ? "• خفضي الإيجار (ابحثي عن موقع أرخص)"
                          : "• Lower rent (find a cheaper location)"}
                      </li>
                    </ul>
                  </div>
                </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <KpiCard
              ...
              hint={
                isAr
                  ? "إجمالي المبيعات الشهرية المتوقعة بعد استقرار المشروع."
                  : "Total expected monthly sales after the project stabilizes."
              }
            />
            <KpiCard
              icon={TrendingUp}
              ...
              hint={
                isAr
                  ? "نسبة صافي الربح من الإيراد. القاعدة: 10% متوسط، 20%+ ممتاز."
                  : "Net profit as % of revenue. 10% is moderate, 20%+ is excellent."
              }
            />
            <KpiCard
              icon={Calendar}
              ...
              hint={
                fs.payback_period_months
                  ? isAr
                    ? "كم شهر تحتاجين لاسترداد رأس المال (مع احتساب خسائر الأشهر الأولى)."
                    : "Months needed to recoup capital (accounting for early-month losses)."
                  : isAr
                    ? "ما يمكن حسابها لأن المشروع خاسر — لازم يكون فيه ربح صافي شهري حتى نقدر نحسب."
                    : "Can't be calculated — the project is losing money, so there's no profit to recoup capital."
              }
            />
            <KpiCard
              icon={Target}
              ...
              hint={
                isAr
                  ? "تقييم السوق المحلي بناءً على المنافسين القريبين وكثافة الطلب."
                  : "Market score based on nearby competitors and demand density."
              }
            />
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <motion.div
              ...
              style={{
                ...
                borderColor: decisionTheme.border,
              }}
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              ...
              <div
                className="text-xl font-bold mb-3 font-[Changa]"
                style={{ color: decisionTheme.text }}
              >
                {clsDisplay}
              </div>
            ...
            >
              ...
              {typeof exec === "object" && exec.verdict ? (
                <>
                  <p className="text-gray-700 leading-relaxed mb-4 font-[Changa]">
                    {exec.verdict}
                  </p>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {exec.key_opportunity && (
                      <div className="bg-green-50 border border-green-200 rounded-lg p-3">
                        ...
                        <div className="text-sm text-gray-700 font-[Changa]">
                          {exec.key_opportunity}
                        </div>
                      </div>
                    )}
                ...
                <p className="text-gray-700 leading-relaxed font-[Changa]">
                  {String(exec)}
                </p>
              )}
            </motion.div>
          ...
          {fs.inputs_summary && (
            ...
            >
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
                <div>
                  <div className="text-xs text-slate-500 font-[Changa] mb-1">
                    {isAr ? "رأس المال" : "Capital"}
                  </div>
                  <div className="text-sm font-bold text-[#08312D] font-[Changa]">
                    {fs.inputs_summary.capital.toLocaleString()}{" "}
                    {isAr ? "ر.س" : "SAR"}
                  </div>
                </div>
                <div>
                  <div className="text-xs text-slate-500 font-[Changa] mb-1">
                    {isAr ? "الإيجار/شهر" : "Rent/mo"}
                  </div>
                  <div className="text-sm font-bold text-[#08312D] font-[Changa]">
                    {fs.inputs_summary.rent.toLocaleString()}{" "}
                    {isAr ? "ر.س" : "SAR"}
                  </div>
                </div>
                <div>
                  ...
                  <div className="text-sm font-bold text-[#08312D] font-[Changa]">
                    {fs.inputs_summary.employees}
                  </div>
                </div>
                <div>
                  <div className="text-xs text-slate-500 font-[Changa] mb-1">
                    {isAr ? "سعر الوجبة" : "Avg Price"}
                  </div>
                  <div className="text-sm font-bold text-[#08312D] font-[Changa]">
                    {fs.inputs_summary.avg_price.toLocaleString()}{" "}
                    {isAr ? "ر.س" : "SAR"}
                  </div>
                </div>
                <div>
                  <div className="text-xs text-slate-500 font-[Changa] mb-1">
                    {isAr ? "العملاء/يوم" : "Customers/day"}
                  </div>
                  <div className="text-sm font-bold text-[#08312D] font-[Changa]">
                    {fs.inputs_summary.customers_per_day}
                  </div>
                </div>
                <div>
                  <div className="text-xs text-slate-500 font-[Changa] mb-1">
                    {isAr ? "الإيراد/شهر (محسوب)" : "Revenue/mo (calc)"}
                  </div>
                  <div className="text-sm font-bold text-[#08312D] font-[Changa]">
                    {(
                      fs.inputs_summary.avg_price *
                      fs.inputs_summary.customers_per_day *
                      28
                    ).toLocaleString()}{" "}
                    {isAr ? "ر.س" : "SAR"}
            ...
            <div className="p-6 lg:p-8">
              ...
              {activeTab === "financial" && (
                <div className="space-y-6">
                  ...
                  {(() => {
                    ...
                    const tiers = [
                      ...
                      },
                      {
                        range: "0% - 3%",
                        label: isAr
                          ? "حدّي قرب الصفر — لا يصمد لأي ضغط"
                          : "Marginal — won't survive shocks",
                        min: 0,
                        max: 3,
                        bg: "bg-orange-100",
                      ...
                      {
                        range: isAr ? "أقل من 0%" : "< 0%",
                        label: isAr
                          ? "خسارة — المصاريف تتجاوز الإيراد"
                          : "Loss",
                        min: -Infinity,
                        max: 0,
                        bg: "bg-red-100",
                    ...
                    return (
                      <div className="bg-blue-50 border border-blue-200 rounded-xl p-5">
                        <div className="flex items-start gap-3 mb-4">
                          ...
                          <div>
                            <h3 className="text-lg font-bold text-blue-900 font-[Changa]">
                              {isAr
                                ? "📚 ما هو هامش الربح؟ (شرح)"
                                : "What is Profit Margin? (Explained)"}
                            </h3>
                        ...
                        <div className="bg-white rounded-lg p-4 mb-4 border border-blue-200">
                          ...
                          <div
                            className="text-sm font-bold text-[#08312D] font-[Changa] mb-3 font-mono"
                            style={{ direction: "ltr" }}
                          >
                            {isAr ? "هامش الربح" : "Margin"} = (
                            {isAr ? "صافي الربح" : "Net Profit"} ÷{" "}
                            {isAr ? "الإيراد" : "Revenue"}) × 100
                          </div>
                          <div className="text-xs text-gray-500 font-[Changa] mb-1 mt-3">
                            {isAr ? "حسابكِ الفعلي" : "Your calculation"}
                          </div>
                          <div
                            className="text-base font-bold text-blue-900 font-[Changa] font-mono"
                            style={{ direction: "ltr" }}
                          >
                            ({fs.monthly_net_profit.toLocaleString()} ÷{" "}
                            {fs.monthly_revenue.toLocaleString()}) × 100 ={" "}
                        ...
                        <div className="text-xs font-bold text-gray-700 mb-2 font-[Changa]">
                          {isAr
                            ? "التقييم في قطاع المطاعم السعودي:"
                            : "Saudi Restaurant Industry Benchmarks:"}
                        </div>
                            ...
                            return (
                              <div
                                ...
                                className={`flex items-center gap-3 px-3 py-2 rounded-lg border ${
                                  isCurrent
                                    ? `${tier.bg} ${tier.border} font-bold`
                                    : "bg-white border-gray-200"
                                }`}
                              >
                                <span
                                  className="text-xs font-mono text-gray-700 min-w-[80px] font-[Changa]"
                                  style={{ direction: "ltr" }}
                                >
                                  {tier.range}
                                </span>
                                <span className="text-xs text-gray-700 flex-1 font-[Changa]">
                                  {tier.label}
                                </span>
                                {isCurrent && (
                                  <span className="text-sm font-[Changa]">
                                    {isAr ? "أنتِ هنا" : "← You"}
                                  </span>
                                )}
                              </div>
                  ...
                  {/* Capital Allocation */}
                  {fs.capital_allocation &&
                    fs.capital_allocation.length > 0 && (
                        ...
                        <table className="w-full text-sm font-[Changa]">
                          <thead>
                            <tr className="border-b border-gray-200 text-gray-500 text-xs">
                              <th
                                className={`py-2 ${isAr ? "text-right" : "text-left"}`}
                              >
                                {isAr ? "البند" : "Item"}
                              </th>
                              <th
                            ...
                            return insufficient ? (
                              <div className="mt-3 bg-red-50 border border-red-200 rounded-lg p-3">
                                <p className="text-sm text-red-800 font-[Changa] font-bold mb-1">
                                  ⚠{" "}
                                  {isAr
                                    ? "تنبيه: الاحتياطي غير كافٍ"
                                    : "Warning: Insufficient cushion"}
                                </p>
                  ...
                  {fs.monthly_projection &&
                    fs.monthly_projection.length > 0 && (
                      <div className="bg-gradient-to-br from-[#FFF9F0] to-white border border-[#C6A75E]/30 rounded-xl p-5">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
                          ...
                          <div className="bg-white rounded-lg p-3 border border-gray-200">
                            ...
                            <div
                              className={`text-lg font-bold font-[Changa] ${(fs.year_1_total_profit ?? 0) >= 0 ? "text-green-700" : "text-red-700"}`}
                            >
                              {(fs.year_1_total_profit ?? 0).toLocaleString()}{" "}
                              {isAr ? "ر.س" : "SAR"}
                            </div>
                          </div>
                        </div>
                        ...
                        <ResponsiveContainer width="100%" height={260}>
                          <LineChart data={fs.monthly_projection}>
                            ...
                            <Tooltip
                              formatter={(v: number) =>
                                v.toLocaleString() + (isAr ? " ر.س" : " SAR")
                              }
                              labelFormatter={(m) =>
                                isAr ? `الشهر ${m}` : `Month ${m}`
                              }
                            />
                            <Legend />
                            <ReferenceLine
                    ...
                    <div className="bg-white border border-gray-200 rounded-xl p-5">
                      <div className="flex items-start gap-3 mb-4">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-gray-600 font-[Changa] mt-1">
                            {isAr
                              ? `الافتراضات: نمو إيراد +${Math.round((fs.yearly_revenue_growth ?? 0.1) * 100)}% سنوياً، تضخم تكاليف ثابتة +${Math.round((fs.yearly_cost_inflation ?? 0.05) * 100)}% سنوياً`
                              : `Assumptions: revenue growth +${Math.round((fs.yearly_revenue_growth ?? 0.1) * 100)}%/year, fixed-cost inflation +${Math.round((fs.yearly_cost_inflation ?? 0.05) * 100)}%/year`}
                          </p>
                        </div>
                      </div>
                            ...
                            {fs.yearly_summary.map((y) => {
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <td
                                    className={`py-3 font-bold ${profitPositive ? "text-green-700" : "text-red-700"}`}
                                  >
                                    {profitPositive ? "+" : ""}
                                    {y.net_profit.toLocaleString()}
                                  </td>
                                  <td
                                    className={`py-3 font-bold ${cumulPositive ? "text-green-700" : "text-red-700"}`}
                                  >
                                    {cumulPositive ? "+" : ""}
                                    {y.cumulative_profit.toLocaleString()}
                                  </td>
                                  <td
                                    className={`py-3 font-bold ${recouped ? "text-green-700" : "text-amber-700"}`}
                                  >
                      ...
                      {/* ROI summary banner */}
                      <div
                        className={`rounded-lg p-4 mb-5 ${(fs.roi_3_year_percent ?? 0) >= 100 ? "bg-green-50 border border-green-200" : "bg-amber-50 border border-amber-200"}`}
                      >
                        <div className="flex items-center gap-3">
                          <Trophy
                            className={`w-6 h-6 ${(fs.roi_3_year_percent ?? 0) >= 100 ? "text-green-700" : "text-amber-700"}`}
                          />
                          <div>
                            <div className="text-xs text-gray-600 font-[Changa]">
                              {isAr
                                ? "العائد على الاستثمار بعد 3 سنوات (ROI)"
                                : "Return on Investment after 3 years"}
                            </div>
                            <div
                              className={`text-2xl font-black font-[Changa] ${(fs.roi_3_year_percent ?? 0) >= 100 ? "text-green-700" : "text-amber-700"}`}
                            >
                              {fs.roi_3_year_percent?.toFixed(1) ?? "—"}%
                              <span className="text-sm font-normal text-gray-600 ml-2">
                                (
                                {(fs.total_3_year_profit ?? 0).toLocaleString()}{" "}
                                {isAr ? "ر.س ربح صافي" : "SAR net"})
                              </span>
                            </div>
                          </div>
                        </div>
                          ...
                          <ResponsiveContainer width="100%" height={280}>
                            <LineChart data={fs.cumulative_profit_curve}>
                              ...
                              <ReferenceLine
                                ...
                                label={{
                                  value: isAr
                                    ? `رأس المال (${(project?.capital || 0).toLocaleString()})`
                                    : `Capital (${(project?.capital || 0).toLocaleString()})`,
                                  fill: "#dc2626",
                                  fontSize: 11,
                              ...
                              {fs.payback_period_months && (
                                <ReferenceLine
                                  ...
                                  label={{
                                    value: isAr
                                      ? `استرداد: ش ${Math.round(fs.payback_period_months)}`
                                      : `Payback: M${Math.round(fs.payback_period_months)}`,
                                    fill: "#22c55e",
                                    fontSize: 11,
                                    position: "top",
                                  }}
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-[#08312D] mb-4 font-[Changa]">
                      {isAr
                        ? "الإيرادات والمصاريف (تشغيل مستقر)"
                        : "Revenue & Expenses (steady-state)"}
                    </h3>
                    <ResponsiveContainer width="100%" height={260}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <StatBox
                      ...
                      value={`${fs.break_even_revenue.toLocaleString()} ${isAr ? "ر.س/شهر" : "SAR/mo"}`}
                    />
                    <StatBox
                      label={isAr ? "فترة الاسترداد" : "Payback Period"}
                      value={
                        fs.payback_period_months
                          ? `${fs.payback_period_months.toFixed(1)} ${isAr ? "شهر" : "months"}`
                          : "—"
                      }
                    />
                  </div>
                        ...
                        return (
                          <div key={item.name}>
                            <div className="flex justify-between text-sm mb-1 font-[Changa]">
                              <span className="font-semibold text-[#08312D]">
                                {item.name}
                              </span>
                              <span className="text-gray-600">
                                {item.value.toLocaleString()}{" "}
                                {isAr ? "ر.س" : "SAR"} ({pct.toFixed(0)}%)
                              </span>
                            </div>
                  ...
                  {fs.stress_test && (
                    <div className="bg-amber-50 border border-amber-200 rounded-xl p-5">
                      <h3 className="text-sm font-bold text-amber-900 mb-4 flex items-center gap-2 font-[Changa]">
                        ⚡{" "}
                        {isAr
                          ? "اختبار الضغط — انخفاض العملاء 10% + ارتفاع التكاليف 10%"
                          : "Stress Test — 10% revenue drop + 10% cost rise"}
                      </h3>
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <StressItem
                          label={
                            isAr ? "الإيراد بعد الضغط" : "Stressed Revenue"
                          }
                          value={`${fs.stress_test.revenue_drop_10pct.toLocaleString()}`}
                        />
                        <StressItem
                          label={
                            isAr ? "التكاليف بعد الضغط" : "Stressed Expenses"
                          }
                          value={`${fs.stress_test.expenses_rise_10pct.toLocaleString()}`}
                        />
                        <StressItem
                          label={isAr ? "صافي الربح" : "Net Profit"}
                          value={`${fs.stress_test.stressed_net_profit.toLocaleString()}`}
                          negative={fs.stress_test.stressed_net_profit < 0}
                        />
                        <StressItem
                          label={isAr ? "الهامش" : "Margin"}
                          value={`${fs.stress_test.stressed_margin_pct.toFixed(1)}%`}
                          negative={fs.stress_test.stressed_margin_pct < 0}
                        />
                      </div>
                    </div>
                  )}
                  ...
                  {fs.improvement_to_18pct_margin && (
                    <div className="bg-green-50 border border-green-200 rounded-xl p-5">
                      <h3 className="text-sm font-bold text-green-900 mb-4 flex items-center gap-2 font-[Changa]">
                        🎯{" "}
                        {isAr
                          ? "المطلوب للوصول إلى هامش 18%"
                          : "Required to reach 18% margin"}
                      </h3>
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <StressItem
                          label={isAr ? "صافي ربح مستهدف" : "Target Profit"}
                          value={`${fs.improvement_to_18pct_margin.target_net_profit.toLocaleString()}`}
                          positive
                        />
                        <StressItem
                          label={
                            isAr ? "الحد الأقصى للمصروفات" : "Max Expenses"
                          }
                          value={`${fs.improvement_to_18pct_margin.max_expenses.toLocaleString()}`}
                          positive
                        />
                        <StressItem
                          label={isAr ? "التوفير المطلوب" : "Required Saving"}
                          value={`${fs.improvement_to_18pct_margin.required_saving.toLocaleString()}`}
                          positive
                        />
                      </div>
                    </div>
              ...
              {activeTab === "market" && ma && (
                <div className="space-y-6">
                  <div className="flex flex-col md:flex-row gap-4 items-stretch">
                    <div
                      className="flex-shrink-0 rounded-2xl p-6 text-white text-center"
                      style={{
                        background: "linear-gradient(135deg, #08312D, #22c55e)",
                      }}
                    >
                      ...
                      <div className="text-xs mt-2 opacity-90 font-[Changa]">
                        {isAr ? "فرصة السوق" : "Market Opportunity"}
                      </div>
                    </div>
                    <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-3">
                      <div className="bg-green-50 rounded-xl p-4 text-center">
                        <div className="text-2xl font-black text-[#08312D]">
                          {ma.direct_competitor_summary.count}
                        </div>
                        <div className="text-xs text-gray-600 mt-1 font-[Changa]">
                          {isAr ? "منافس مباشر" : "Direct competitors"}
                        </div>
                      </div>
                      <div className="bg-amber-50 rounded-xl p-4 text-center">
                        <div className="text-2xl font-black text-amber-700">
                          {ma.direct_competitor_summary.avg_rating || "—"}
                        </div>
                        <div className="text-xs text-gray-600 mt-1 font-[Changa]">
                      ...
                      <div className="bg-gray-50 rounded-xl p-4 flex flex-col items-center justify-center">
                        <div
                          className={`inline-block px-3 py-1 rounded-full text-xs font-bold font-[Changa] ${
                            ["منخفض", "Low"].includes(ma.competition_level)
                              ? "bg-green-100 text-green-700"
                              : ["متوسط", "Moderate"].includes(
                        ...
                        <div className="text-xs text-gray-600 mt-2 font-[Changa]">
                          {isAr ? "مستوى المنافسة" : "Competition"}
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-gray-50 rounded-xl p-5">
                    ...
                    <p className="text-gray-700 leading-relaxed text-sm font-[Changa]">
                      {ma.narrative}
                    </p>
                    <div className="mt-4 pt-4 border-t border-gray-200 grid grid-cols-1 md:grid-cols-2 gap-3 text-sm font-[Changa]">
                      <div>
                        <span className="font-bold text-[#08312D]">
                          🏆 {isAr ? "أقوى منافس:" : "Strongest:"}{" "}
                        </span>
                        <span className="text-gray-700">
                          {ma.direct_competitor_summary.strongest_name}
                        </span>
                      </div>
                      <div>
                        <span className="font-bold text-[#08312D]">
                        ...
                        <span className="text-gray-700">
                          {ma.direct_competitor_summary.weakest_gap}
                        </span>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="bg-white border border-gray-200 rounded-xl p-5">
                      ...
                      <ul className="space-y-2">
                        {ma.bullets.map((b, i) => (
                          <li
                            key={i}
                            className="flex gap-2 text-sm text-gray-700 font-[Changa]"
                          >
                    ...
                    <div className="bg-white border border-gray-200 rounded-xl p-5">
                      ...
                      <ol className="space-y-2">
                        {ma.recommendations.map((r, i) => (
                          <li
                            key={i}
                            className="flex gap-3 text-sm text-gray-700 font-[Changa]"
                          >
                            <span className="w-6 h-6 rounded-full bg-[#08312D] text-white flex items-center justify-center text-xs font-bold flex-shrink-0">
                              {i + 1}
                            </span>
                            <span>{r}</span>
                          </li>
                  ...
                  {ma.classified_competitors &&
                    ma.classified_competitors.length > 0 && (
                      <div className="bg-white border border-gray-200 rounded-xl p-5">
                          ...
                          <table className="w-full text-sm">
                            ...
                            <tbody>
                              {[...ma.classified_competitors]
                                .sort(
                                  (a, b) =>
                                    Number(b.is_direct_competitor) -
                                    Number(a.is_direct_competitor),
                                )
                                .map((c) => {
                                  const place = report.competitor_places?.find(
                                    (p) => p.id === c.id,
                                  );
                                  return (
                                    <tr
              ...
              {activeTab === "market" && !ma && (
                <EmptyState
                  ...
                  message={
                    isAr
                      ? "لا يوجد تحليل سوق لهذه الدراسة"
                      : "No market analysis for this report"
                  }
                />
              )}
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start justify-between gap-3 mb-2">
                          <h4 className="font-bold text-[#08312D] text-sm font-[Changa]">
                            {r.risk}
                          </h4>
                          {sev && (
                            <span
                              className={`${theme.badge} px-3 py-1 rounded-full text-xs font-bold whitespace-nowrap font-[Changa]`}
                            >
                              {tr(sev, language)}
                            </span>
                          )}
              ...
              {activeTab === "steps" && (
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <ul className="space-y-2">
                      {dec.reasons.map((r, i) => (
                        <li
                          key={i}
                          className="flex gap-2 bg-gray-50 border border-gray-200 rounded-lg p-3 text-sm text-gray-700 font-[Changa]"
                        >
                  ...
                  {dec.invest_conditions?.length ||
                  dec.reject_conditions?.length ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {dec.invest_conditions &&
                        dec.invest_conditions.length > 0 && (
                          <div className="bg-green-50 border border-green-200 rounded-xl p-5">
                            ...
                            <ul className="space-y-2">
                              {dec.invest_conditions.map((c, i) => (
                                <li
                                  key={i}
                                  className="text-sm text-gray-700 font-[Changa]"
                                >
                      ...
                      {dec.reject_conditions &&
                        dec.reject_conditions.length > 0 && (
                  ...
                  <div>
                    ...
                    <ol className="space-y-3">
                      {report.next_steps.map((s, i) => (
                        <li
                          key={i}
                          className="flex gap-3 bg-[#FFF9F0] border border-[#C6A75E]/30 rounded-lg p-4 text-sm text-gray-700 font-[Changa]"
                        >
                          <span className="w-7 h-7 rounded-full bg-[#08312D] text-white flex items-center justify-center text-xs font-bold flex-shrink-0">
                            {i + 1}
                          </span>
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm font-[Changa]">
              <InfoRow
                ...
                value={tr(bo.business_type, language)}
              />
              {bo.restaurant_type && (
                <InfoRow
                  label={isAr ? "التخصص" : "Specialty"}
                  value={bo.restaurant_type}
                />
              )}
              <InfoRow
                label={isAr ? "المدينة" : "City"}
                value={tr(bo.city, language)}
              />
              <InfoRow
                label={isAr ? "العملاء المستهدفون" : "Target Customers"}
                value={trEmbeddedCities(bo.target_customers, language)}
              />
              <InfoRow
                label={isAr ? "عرض القيمة" : "Value Proposition"}
                value={trEmbeddedCities(bo.value_proposition, language)}
              />
              {bo.main_products && bo.main_products.length > 0 && (
                <InfoRow
                  label={isAr ? "المنتجات الرئيسية" : "Main Products"}
                  value={bo.main_products.join(" · ")}
                />
              )}
            </div>
          </motion.div>
    ...
    >
      <div className="flex items-start justify-between">
        <div className="flex-1 min-w-0">
          ...
          <div className="text-xl font-black text-[#08312D] font-[Changa] truncate">
            {value}
          </div>
        </div>
        <div
...
function RampStat({
  ...
  return (
    <div
      className={`rounded-lg p-3 border ${highlight ? "bg-[#08312D] border-[#08312D] text-white" : "bg-white border-gray-200"}`}
    >
      <div
        className={`text-xs mb-1 font-[Changa] ${highlight ? "text-white/70" : "text-gray-500"}`}
      >
        {label}
      </div>
      <div
        className={`text-sm font-bold font-[Changa] ${highlight ? "text-white" : "text-[#08312D]"}`}
      >
        {revenue.toLocaleString()} {isAr ? "ر.س" : "SAR"}
      </div>
      <div
        className={`text-xs mt-1 font-[Changa] ${
          highlight
            ? positive
              ? "text-green-300"
              : "text-red-300"
      ...
      >
        {positive ? "+" : ""}
        {profit.toLocaleString()} {isAr ? "ربح" : "profit"}
      </div>
    </div>
  );
}
...
function StressItem({
  ...
  return (
    <div className="text-center">
      <div className="text-lg font-bold font-[Changa]" style={{ color }}>
        {value}
      </div>
