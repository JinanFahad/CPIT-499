...
// Firebase Authentication state listener (signOut is handled by the
...
// Centralized auth-storage helpers (getUserName + logout)
...
export function Header() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      ...
      const name =
        user?.displayName ||
        getUserName(isAr ? "المستخدم" : "User");
      setUserName(name);
    });
  ...
  // ── Logout function ────────────────────────────────────────────────
  // Delegates the actual sign-out + storage cleanup to the shared helper,
  // then redirects to the landing page (which the helper does not do).
